        ...
        <div class="row align-items-center">
            <div class="col-sm-6">
                ...
                <div class="breadcrumbs-area clearfix">
                    <h4 class="page-title pull-left">Feature Edit</h4>
                    <ul class="breadcrumbs pull-left">
                        <li><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                        <li><a href="{{ route('admin.feature.index') }}">All Features</a></li>
                        <li><span>Edit Feature - {{ $feature->title }}</span></li>
                    </ul>
                </div>
            </div>
    ...
    <div class="main-content-inner">
        <div class="row">
            ...
            <div class="col-12 mt-5">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title">Edit Feature - {{ $feature->title }}</h4>
                                    ...
                                    <div class="tab-pane fade {{ $loop->index == 0 ? 'show active' : '' }}" id="{{ $key }}" role="tabpanel"
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-6 col-sm-12">
                                                ...
                                                <input feature="text" class="form-control" name="title[{{ $key }}]" value="{{ $feature->translations['title'][$key]??null }}">
                                            </div>
                                    ...
                                    <div class="form-group col-md-6 col-sm-12">
                                        ...
                                        <select feature="text" class="form-control" name="type">
                                            @foreach (config('app.types') as $key => $value)
                                                <option {{$key==$feature->type?'selected':''}} value="{{ $key }}">{{ $value }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group col-md-6 col-sm-12">
                                        ...
                                        <select feature="text" class="form-control" name="type">
                                            ...
                                            @foreach ($feature_category as $item)
                                                <option {{$item->id==$feature->category?'selected':''}} value="{{ $item->id }}">{{ $item->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
